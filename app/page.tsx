...
const others = [
  ...
  {name:'NDAX',fee:'0.20% flat',coins:'50+',bestFor:'Low-fee traders',rating:4.1,stars:'★★★★☆',pros:['Flat 0.20% trading fee','Staking available','Advanced order types','FINTRAC + CSA registered'],cons:['Less beginner-friendly UI','Lower liquidity on some pairs'],province:null,url:'https://web.newton.co/r/XH1RIR'},
  {name:'Shakepay',fee:'0% + spread',coins:'BTC, ETH',bestFor:'Bitcoin beginners',rating:4.3,stars:'★★★★☆',pros:['Zero explicit fees','Shakesats Bitcoin cashback','Instant Interac e-Transfer','Simplest app in Canada'],cons:['BTC and ETH only','Spread cost hidden in price'],province:null,url:'https://ref.ndax.io/NLZU/rztuxovh'},
    ...
    <div style={{fontFamily:C.font,background:C.bg,color:C.white,minHeight:'100vh'}}>
      ...
      <div style={{maxWidth:1200,margin:'0 auto',padding:'64px 32px 72px',display:'grid',gridTemplateColumns:'1fr 420px',gap:64,alignItems:'center'}}>
        ...
        <div style={{background:C.surface,border:'1px solid '+C.border,borderRadius:24,padding:32,boxShadow:'0 32px 80px rgba(0,0,0,0.5)'}}>
          ...
          {[
            {icon:'₿',bg:'rgba(247,147,26,0.12)',color:'#f7931a',name:'Bitcoin',sym:'BTC',data:btc},
            {icon:'Ξ',bg:'rgba(98,126,234,0.12)',color:'#627eea',name:'Ethereum',sym:'ETH',data:eth}
          ].map((coin,i)=>(
            <div key={coin.sym} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'18px 0',borderBottom:i===0?'1px solid '+C.border:'none'}}>
      ...
      <div style={{borderTop:'1px solid '+C.border,borderBottom:'1px solid '+C.border,background:C.surface}}>
        <div style={{maxWidth:1200,margin:'0 auto',padding:'0 32px',display:'grid',gridTemplateColumns:'repeat(4,1fr)'}}>
          {[['6+','FINTRAC-registered exchanges reviewed'],['100%','CAD on-ramps — no USD conversion needed'],['50%','Capital gains inclusion rate for 2025'],['$0','Minimum deposit on most platforms']].map(([n,l],i)=>(
            <div key={l} style={{padding:'26px 20px',textAlign:'center',borderRight:i<3?'1px solid '+C.border:'none'}}>
              <div style={{fontSize:'1.9rem',fontWeight:800,color:C.emerald,letterSpacing:'-0.02em'}}>{n}</div>
              <div style={{fontSize:12,color:C.muted,marginTop:6,lineHeight:1.4}}>{l}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{maxWidth:1200,margin:'0 auto',padding:'72px 32px 80px'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'baseline',marginBottom:36}}>
          <div>
            <h2 style={{fontSize:'1.85rem',fontWeight:800,letterSpacing:'-0.03em',margin:'0 0 6px'}}>Best Canadian Crypto Exchanges 2026</h2>
            <p style={{fontSize:14,color:C.muted,margin:0}}>All FINTRAC-registered and CSA-authorized. Verified April 2026.</p>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 280px 200px',gap:44,alignItems:'start',position:'relative'}}>
            <div>
              ...
              <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                {['✓ FINTRAC','✓ CSA Registered','✓ Cold Storage Insurance','✓ All Provinces'].map(b=>(
                  <span key={b} style={{fontSize:12,fontWeight:600,padding:'5px 12px',borderRadius:8,background:C.emeraldDim,color:C.emerald,border:'1px solid '+C.emeraldBorder}}>{b}</span>
                ))}
              </div>
            </div>
