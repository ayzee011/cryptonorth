...
const exchanges = [{
    name: 'Bitbuy', featured: true, rating: 'âââââ', score: '4.8',
  ...
  {
    name: 'Newton', featured: false, rating: 'âââââ', score: '4.4',
    fee: '0.25â0.6%', coins: '70+', bestFor: 'Intermediate',
    pros: ['70+ coins', 'Free Interac e-Transfer'],
    cons: ['No staking rewards'],
    url: 'https://newton.co',
  },
  {
    name: 'NDAX', featured: false, rating: 'âââââ', score: '4.1',
  ...
  {
    name: 'Shakepay', featured: false, rating: 'âââââ', score: '4.3',
  ...
  {
    name: 'Wealthsimple', featured: false, rating: 'âââââ', score: '4.2',
    fee: '1.5â2%', coins: '50+', bestFor: 'Existing WS users',
    pros: ['Trusted brand', 'FINTRAC registered', 'Easy UI'],
    cons: ['Higher fees'],
    url: 'https://wealthsimple.com/crypto',
  },
  {
    name: 'Kraken', featured: false, rating: 'âââââ', score: '4.7',
    fee: '0.16â0.26%', coins: '400+', bestFor: 'Advanced traders',
...
export default function Home() {
  const [btcPrice, setBtcPrice] = useState('â')
  const [btcChg, setBtcChg] = useState('â')
  const [btcUp, setBtcUp] = useState(true)
  const [ethPrice, setEthPrice] = useState('â')
  const [ethChg, setEthChg] = useState('â')
    ...
    <div style={{ position: 'relative', zIndex: 1 }}>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 100, background: 'rgba(11,18,32,0.92)', backdropFilter: 'blur(16px)', borderBottom: `1px solid ${C.border}` }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 68, gap: 24 }}>
          <a href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: C.font, fontSize: '1.15rem', fontWeight: 700, flexShrink: 0 }}>
            <div style={{ width: 34, height: 34, background: C.emerald, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 17 }}>ð</div>
          ...
          <a href="/exchanges" style={{ background: C.emerald, color: '#fff', padding: '0 20px', height: 38, borderRadius: 9999, fontWeight: 600, fontSize: 13.5, display: 'flex', alignItems: 'center', whiteSpace: 'nowrap', flexShrink: 0 }}>Compare Canadian Exchanges â</a>
        </div>
      </nav>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '96px 32px 80px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: C.emeraldDim, border: `1px solid ${C.emeraldBorder}`, color: C.emerald, fontSize: 11.5, fontWeight: 600, borderRadius: 9999, padding: '6px 14px', marginBottom: 28, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
            ð Built for Canadians &nbsp;Â·&nbsp; CRA &amp; CSA Compliant
          </div>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            <a href="/exchanges" style={{ background: C.white, color: '#0b1220', padding: '13px 26px', borderRadius: 9999, fontWeight: 700, fontSize: 14, display: 'inline-block' }}>Find the Best Exchange â</a>
            <a href="/tax-guide" style={{ background: 'transparent', color: C.white, padding: '13px 26px', borderRadius: 9999, fontWeight: 600, fontSize: 14, border: '1.5px solid rgba(255,255,255,0.18)', display: 'inline-block' }}>2026 Tax Guide</a>
          </div>
        </div>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 20, padding: 28, boxShadow: '0 32px 80px rgba(0,0,0,0.5)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <span style={{ fontSize: 11, color: C.dim, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Market Snapshot Â· CAD</span>
          ...
          {[
            { icon: 'â¿', bg: 'rgba(247,147,26,0.15)', color: '#f7931a', name: 'Bitcoin', sym: 'BTC', price: btcPrice, chg: btcChg, up: btcUp },
            { icon: 'Î', bg: 'rgba(98,126,234,0.15)', color: '#627eea', name: 'Ethereum', sym: 'ETH', price: ethPrice, chg: ethChg, up: ethUp },
          ].map((coin, i) => (
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '72px 32px' }} id="exchanges">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
          ...
          <a href="/exchanges" style={{ color: C.emerald, fontSize: 13.5, fontWeight: 600 }}>Full comparison â</a>
        </div>
            ...
            <div key={e.name} style={{ background: e.featured ? C.surface2 : C.surface, border: `1px solid ${e.featured ? C.emeraldBorder : C.border}`, borderRadius: 18, padding: 26, display: 'flex', flexDirection: 'column', gap: 14, transition: 'transform .2s' }}>
              <div>
                <div style={{ fontFamily: C.font, fontWeight: 700, fontSize: 18 }}>
                  {e.name}
                  {e.featured && <span style={{ fontSize: 10, color: C.emerald, background: C.emeraldDim, padding: '2px 8px', borderRadius: 4, marginLeft: 8, fontWeight: 600 }}>â TOP PICK</span>}
                </div>
              ...
              <div>
                {e.pros.map(p => <div key={p} style={{ fontSize: 12, color: C.emerald }}>â {p}</div>)}
                {e.cons.map(c => <div key={c} style={{ fontSize: 12, color: '#ef4444' }}>â {c}</div>)}
              </div>
              ...
              <div style={{ fontSize: 10, color: C.dim, textAlign: 'center' }}>Affiliate link â we may earn a commission</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '72px 32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
          {[
            { icon: 'ð', title: 'Live CAD Prices', desc: 'Real-time prices for 500+ cryptocurrencies in Canadian dollars, updated every 30 seconds.' },
            { icon: 'ð', title: 'CRA Tax Ready', desc: 'Plain-English guidance on reporting crypto on your T1. Capital gains vs business income explained.' },
            { icon: 'ð', title: 'Secure Canadian On-Ramps', desc: 'Only FINTRAC-registered exchanges. Your money stays in Canada with real investor protections.' },
          ].map(f => (
      ...
      <div id="tax" style={{ background: C.surface, borderTop: `1px solid ${C.border}`, borderBottom: `1px solid ${C.border}`, padding: '80px 32px', textAlign: 'center' }}>
        ...
        <p style={{ color: C.muted, fontSize: 16, maxWidth: 440, margin: '0 auto 32px' }}>Compare Canadian crypto exchanges side by side â fees, coins, security, and FINTRAC status.</p>
        <a href="#exchanges" style={{ background: C.emerald, color: '#fff', padding: '15px 36px', borderRadius: 9999, fontWeight: 700, fontSize: 15, display: 'inline-block' }}>Compare Canadian Exchanges â</a>
      </div>
      ...
      <footer style={{ background: '#070e1a', borderTop: `1px solid ${C.border}`, padding: '44px 32px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 20 }}>
          <div>
            <div style={{ fontFamily: C.font, fontWeight: 700, fontSize: 15 }}>ð CryptoNorth</div>
          ...
          <div style={{ fontSize: 11, color: '#2a3a52', width: '100%', marginTop: 8 }}>
            Â© 2026 CryptoNorth. Not financial advice. Some links are affiliate links â we may earn a commission at no extra cost to you.
          </div>
        </div>
      </footer>
